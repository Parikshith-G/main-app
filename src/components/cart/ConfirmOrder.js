...
const ConfirmOrder = () => {
  ...
  const itemsPrice = cartItems.reduce(
    (acc, item) => acc + item.price * 1 * item.quantity,
    0
  );
  const shippingPrice = itemsPrice > 50 ? 0 : (50 * 1) / 200;
              ...
              <div className="cart-item my-1" key={item.product}>
                <div className="row">
                  ...
                  <div className="col-4 col-lg-4 mt-4 mt-lg-0">
                    <p>
                      {item.quantity} x Rs {item.price * 1} ={" "}
                      <b>Rs {(item.quantity * item.price * 1).toFixed(2)}</b>
                    </p>
                  </div>
                </div>
              </div>
